...
func (f *Forwarder) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if written != 0 {
        ...
        tplcontent, err := http.Get("http://localhost/portal/")
           if err != nil {
                return
           }
        defer tplcontent.Body.Close()
        tplbody, err := ioutil.ReadAll(tplcontent.Body)
        clen := len(tplbody)
        clen += 10
	written2, _ := io.Copy(w, tplcontent.Body)

        w.Header().Set("Content-Length", strconv.Itoa(clen))	
	if written2 != 0 {
		w.Header().Set(ContentLength, strconv.FormatInt(written2, clen))
	}
